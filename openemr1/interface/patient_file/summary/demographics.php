...
<head>
    ...
    function oldEvt(apptdate, eventid) {
        ...
        dlgopen('../../main/calendar/add_edit_event.php?date=' + encodeURIComponent(apptdate) + '&eid=' +
            encodeURIComponent(eventid), '_blank', 725, 500, '', title);
    }
    ...
    function deleteme() { // @todo don't think this is used any longer!!
        dlgopen('../deleter.php?patient=' + < ? php echo js_url($pid); ? > +'&csrf_token_form=' + < ? php echo js_url(
            CsrfUtils::collectCsrfToken()); ? > , '_blank', 500, 450, '', '', {
            allowResize: false,
            allowDrag: false,
            dialogId: 'patdel',
            type: 'iframe'
    ...
    function editScripts(url) {
        var AddScript = function() {
            ...
            iam.location.href = '<?php echo $GLOBALS['
            webroot ']?>/controller.php?prescription&edit&id=0&pid=' + < ? php echo js_url($pid); ? > ;
        };
        var ListScripts = function() {
            ...
            var iam = top.tab_mode ? top.frames.editScripts : window[1];
            iam.location.href = '<?php echo $GLOBALS['
            webroot ']?>/controller.php?prescription&list&id=' + < ? php echo js_url($pid); ? > ;
        };
    ...
    $(document).ready(function() {
        ...
        if ($GLOBALS['erx_enable']) {
            ...
            $soap_status = sqlStatement(
                "select soap_import_status,pid from patient_data where pid=? and soap_import_status in ('1','3')",
                array($pid));
        ...
        while ($gfrow = sqlFetchArray($gfres)) {
            ?
            >
            $( < ? php echo js_escape("#".$gfrow['grp_form_id'].
                "_ps_expand"); ? > ).load("lbf_fragment.php?formname=" + < ? php echo js_url($gfrow[
                'grp_form_id']); ? > , {
                csrf_token_form: < ? php echo js_escape(CsrfUtils::collectCsrfToken()); ? >
            }); <
            ?
        ...
        if ($GLOBALS['patient_birthday_alert']) {
            ...
            elseif($active_reminders || $all_allergy_alerts) {
                    ?
                    >
                    openReminderPopup(); <
                    ?
    ...
    function setMyPatient() {
        ...
        if (isset($_GET['set_pid'])) {
            ...
            parent.left_nav.setPatient( < ? php echo js_escape($result['fname'].
                    " ".$result['lname']).
                ",".js_escape($pid).
                ...
                if (empty($date_of_death)) {
                    echo js_escape(" ".xl('DOB').
                        ...
                        " ".xl('Age').
                        ": ".getPatientAgeDisplay($result['DOB_YMD']));
                } else {
                    echo js_escape(" ".xl('DOB').
                        ": ".oeFormatShortDate($result['DOB_YMD']).
                        " ".xl('Age at death').
            ...
            if (sqlNumRows($result4) > 0) {
                while ($rowresult4 = sqlFetchArray($result4)) {
                    ...
                    EncounterDateArray[Count] = < ? php echo js_escape(oeFormatShortDate(date("Y-m-d", strtotime(
                        $rowresult4['date'])))); ? > ;
                    CalendarCategoryArray[Count] = < ? php echo js_escape(xl_appt_category($rowresult4[
                        'pc_catname'])); ? > ;
                    Count++; <
                    ?
                    php
        ...
        if ((isset($_GET['set_pid'])) && (isset($_GET['set_encounterid'])) && (intval($_GET['set_encounterid']) > 0)) {
            ...
            >
            encurl = 'encounter/encounter_top.php?set_encounter=' + < ? php echo js_url($encounter); ? > +'&pid=' + < ?
            php echo js_url($pid); ? > ; <
            ?
            php
            if ($GLOBALS['new_tabs_layout']) {
                ?
                >
                parent.left_nav.setEncounter( < ? php echo js_escape(oeFormatShortDate(date("Y-m-d", strtotime(
                    $query_result['date'])))); ? > , < ? php echo js_escape($encounter); ? > , 'enc');
                top.restoreSession();
                ...
                parent.left_nav.setEncounter( < ? php echo js_escape(attr(oeFormatShortDate(date("Y-m-d", strtotime(
                    $query_result['date']))))); ? > , < ? php echo js_escape($encounter); ? > , othername);
                top.restoreSession();
...
<body class="body_top patient-demographics">
    <div id="container_div" class="<?php echo $oemr_ui->oeContainer();?>">
        ...
        <a href='../birthday_alert/birthday_pop.php?pid=<?php echo attr_url($pid); ?>&user_id=<?php echo attr_url($_SESSION['authId']); ?>'
                                ...
                                <table border='0' cellpadding='0' width='100%'>
                                    ...
                                    <tr>
                                        <td valign='top'>
                                            <span class='text'>
                                                ...
                                                <?php echo text($row['group_number']); ?>
                                            </span>
                                        </td>
                                        <td valign='top'>
                                            <span class='bold'><?php echo xlt('Subscriber'); ?>: </span><br>
                                            <span
                                                class='text'><?php echo text($row['subscriber_fname'] . ' ' . $row['subscriber_mname'] . ' ' . $row['subscriber_lname']); ?>
                                                <?php
                                        ...
                                        <td valign='top'>
                                            ...
                                            <br><span class='bold'><?php echo xlt('Subscriber Employer'); ?>:
                                            </span><br>
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <span
                                                class='text'><?php echo text($policy_types[$row['policy_type']]); ?></span>
                                            <?php } ?>
                                        </td>
                                        <td valign='top'></td>
                                        <td valign='top'></td>
